...
export default function ProductForm({ clientId, product, onBack, onSaved }: {
  clientId: string
  product?: Product
  onBack: () => void
  onSaved: () => void
  ...
  const handlePhotoAdd = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPhotoFiles((prev) => [...prev, ...files.map((file) => ({ file, localUrl: URL.createObjectURL(file) }))])
  ...
  const deleteExistingPhoto = async (url: string) => {
    if (!product) return
    try {
      await axios.delete(`/api/admin/products/${product.id}/photos`, { params: { url } })
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setError('')
    try {
      const payload = { ...data, price: data.price ? parseFloat(data.price) : undefined, clientId }

      const saved = product
        ? (await axios.put<Product>(`/api/admin/products/${product.id}`, payload)).data
        : (await axios.post<Product>('/api/admin/products', payload)).data

      const upload = async <T>(endpoint: string, file: File) => {
        const fd = new FormData()
        fd.append('file', file)
        return (await axios.post<T>(`/api/admin/products/${saved.id}/${endpoint}`, fd)).data
      }
  ...
  return (
    <div>
      <button onClick={onBack} className="text-gray-500 hover:text-gray-800 text-sm mb-6 transition-colors">
        ← Назад к списку
      </button>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl border p-6 space-y-5 max-w-lg">
        <div>
        ...
        <div>
          ...
          <textarea {...register('description')} rows={3} className={`${inputCls} resize-none`} placeholder="Краткое описание товара" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input {...register('category')} className={inputCls} placeholder="Мебель, Кресла..." />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">3D модель (.glb)</label>
          {product?.modelUrl && <p className="text-xs text-green-600 mb-1.5">✓ Загружена: {product.modelUrl}</p>}
          <input type="file" accept=".glb" onChange={(e) => setModelFile(e.target.files?.[0] ?? null)} className={fileCls('blue')} />
        ...
        <div>
          ...
          {(previewLocalUrl || currentImageUrl) && (
            <div className="mb-2 relative inline-block">
              <img src={previewLocalUrl ?? currentImageUrl} alt="preview" className="h-24 w-24 object-cover rounded-lg border" />
              {previewLocalUrl && (
                <span className="absolute -top-1.5 -right-1.5 bg-blue-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">новое</span>
              )}
            </div>
          )}
          <input type="file" accept="image/*" onChange={handlePreviewChange} className={fileCls('green')} />
          <p className="text-xs text-gray-400 mt-1">JPG, PNG, WebP · Максимум 10 МБ</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Фотографии товара</label>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {photoFiles.map(({ localUrl }, idx) => (
                <div key={localUrl} className="relative group">
                  <img src={localUrl} alt="" className="h-20 w-20 object-cover rounded-lg border border-dashed border-blue-300" />
                  <span className="absolute -top-1.5 -left-1.5 bg-blue-500 text-white text-[10px] font-bold px-1 py-0.5 rounded-full">+</span>
                  <button
                    type="button"
          ...
          <input ref={photoInputRef} type="file" accept="image/*" multiple onChange={handlePhotoAdd} className={fileCls('purple')} />
          <p className="text-xs text-gray-400 mt-1">Можно выбрать несколько файлов</p>
        </div>

        <div className="flex items-center gap-2">
          <input {...register('isActive')} type="checkbox" id="isActive" className="w-4 h-4 text-blue-600 rounded" />
